...
在项目根目录的 .env 定义的配置项，并都会加载到环境变量中，通过 `process.env.{配置KEY}` 获取。
该文件不在版本库中，不依赖环境，示例配置参考 `doc/sample.env` 文件

并没有采用官方推荐的 Config 模块方案，一是因为依赖注入在有些场景中不好实现，例如日志初始化文件中读取配置，二是依赖注入读取太繁琐。如果您的配置来源比较复杂，例如请求配置中心、数据库等，建议参考官方的实现。
...
RBAC（基于角色的权限控制）是企业软件常用的权限管理技术，目前仅提供了鉴权相关的守卫和装饰器，RBAC 鉴权逻辑请自行实现。
